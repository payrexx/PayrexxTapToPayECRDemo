package com.payrexx.taptopayecr.viewholder;
...
import com.payrexx.taptopayecr.TransactionActivity;
...
public class TransactionListItem extends RecyclerView.ViewHolder {
    ...
    public void setData(Transaction transaction) {
        ...
        ((TextView) this.itemView.findViewById(R.id.amount)).setText("CHF " + transaction.amount.toString());
        LocalDateTime dateTime = LocalDateTime.parse(transaction.date, Transaction.dateTimeFormatter);
        ((TextView) this.itemView.findViewById(R.id.date)).setText(dateTime.format(DateTimeFormatter.ofLocalizedDateTime(FormatStyle.SHORT)));
        String methodText = transaction.paymentMethod.toString();
        if (transaction.cardType != null && transaction.cardType.length() > 0) {
